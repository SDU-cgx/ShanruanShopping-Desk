    ...
    <div class="commodity">
        ...
        <div v-if="commodities.length">
            <div class="commoidtySort">
                <div class="sort">
                    <a href="javascript:;" class="bt curr" @click="handleGetCommodityList(0, 'price', 0)"><span>综合</span><i
                            class="el-icon-d-caret"></i></a>
                    <a href="javascript:;" class="bt" @click="handleGetCommodityList('1', 'inventory', 1)"><span>销量</span><i
                            class="el-icon-d-caret"></i></a>
                    <a href="javascript:;" class="bt"
                        @click="handleGetCommodityList('1', 'evaluation_count', 2)"><span>评论数</span><i
                            class="el-icon-d-caret"></i></a>
                    <a href="javascript:;" class="bt" @click="handleGetCommodityList('1', 'price', 3)"><span>价格</span><i
        ...
        <commodityList v-if="sell" @update="update" @updateCommodity='updateCommodity' :list="commodities"
            :isNeedMask="true" /> <!-- 商家加载 -->
        <Dialog v-if="sell" @update="update" @dialogHide="dialogHide" :dialogVisible="dialogVisible" 
        :addFlag="false"  :form="form" /> <!-- 只有商家和管理员加载 -->
        ...
        <h1>{{ message }}</h1>
        <el-pagination :hide-on-single-page="commodities.length == 0" @size-change="handleSizeChange"
    ...
    methods: {
        ...
        handleCurrentChange(val) {
            ...
            if (this.state.index != -1) {   //说明处于排序状态
                this.getCommodityList(this.sortType, this.state.key, this.state.index);
            } else {
        ...
        nextPage() {
         ...
         loadCommodities() {
            if (this.sell && this.$route.path.indexOf("/shop") != -1) {
                // 如果是商家页面，则加载商家商品
                this.getCommoditiesByShop(this.sell.id);
            } else {
                // 如果是用户页面，则加载所有商品
