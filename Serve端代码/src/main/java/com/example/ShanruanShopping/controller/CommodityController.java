...
import com.example.ShanruanShopping.service.CommodityService;
...
public class CommodityController extends ApiController {
    ...
    @PostMapping("searchOrderNoPage")
    public R searchOrderNoPage(@RequestBody Map<String, String[]> map) {
        int page = 1;
        int status = 1;
        if (mapExist(map, "status")) status = Integer.parseInt(map.get("status")[0]);
        QueryWrapper<Commodity> wrapper = new QueryWrapper<Commodity>().eq("status", status);
        ...
        if (mapExist(map, "asc")) wrapper.orderByAsc(map.get("asc")); //关键列升序
        if (mapExist(map, "desc")) wrapper.orderByDesc(map.get("desc")); //关键列降序
        return success(this.commodityService.page(new Page<>(page, 100), wrapper)); //返回列表
    }
    ...
    public R delete(@RequestBody Commodity commodity) {
        return success(this.commodityService.removeById(commodity));
    }

    @GetMapping("commodityNum")
    public R count() {
        return success(this.commodityService.getCommodityCount());
    }

//    @GetMapping("getPhoto")
//    public R selectPhoto(@RequestParam("id") long id) {
//        return success();
//    }
